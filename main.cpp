...
void MultiplyMatrix( const CImg<double> matrix,const CImg<unsigned char> cropedCompress , CImg<unsigned char>  cropedOrigin){
	
	for(int i =0 ; i< BLOC_SIZE ;++i){
		for(int j=0; j<= BLOC_SIZE;++j){
			cropedOrigin(i,j) = cropedCompress(i,j) * matrix(i,j);
		}
	}	
}

double ComputeInvertDctPoint(const CImg<unsigned char> bloc, int i, int j){
	#define C(i) ((i)?1:(1/sqrt(2)))
	double sum = 0;
...
void ComputeInvertDctBloc( CImg<unsigned char> bloc,CImg<double>& cbloc ){
		for ( int i = 0; i < BLOC_SIZE; ++i ) {
			for ( int j = 0; j < BLOC_SIZE; ++j ){
				cbloc ( i,j) = ComputeInvertDctPoint(bloc,i,j);
			}
		}
		for ( int x = 0; x < BLOC_SIZE; ++x ) {
			for ( int y = 0; y < BLOC_SIZE; ++y ) {
				cbloc( x, y ) = cbloc( x, y ) + 128;
		}
	}
}
...
CImg <unsigned char> JPEGDecoder (const CImg<unsigned char> compressedImage, const float q){
	...
	for ( int i = 0; i < compressedImage.width(); i += BLOC_SIZE ) {
		for ( int j = 0; j < compressedImage.height(); j += BLOC_SIZE ) {
			MultiplyMatrix(Q,compressedImage.get_crop(i, j, i + BLOC_SIZE, j + BLOC_SIZE ), decompressImage.get_crop(i, j, i + BLOC_SIZE, j + BLOC_SIZE ));
			ComputeInvertDctBloc(decompressImage.get_crop(i, j, i + BLOC_SIZE, j + BLOC_SIZE ),fullPrecision);
		}
